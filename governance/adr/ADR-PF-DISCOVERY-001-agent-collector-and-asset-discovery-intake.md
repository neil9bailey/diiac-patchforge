...
5. A day-1 Windows/Linux collector CLI implemented as an outbound-only client.
...
- Run a collector on Windows or Linux using a local JSON configuration and environment-backed bearer token.
...
- A Windows/Linux collector CLI can register/update itself, create/update a read-only policy, collect supported asset categories, and push the resulting run into PatchForge Core.
...
The tradeoff is increased security responsibility: collector identity, package signing, credential handling, tenant isolation, and audit evidence must be treated as high-risk product areas.
